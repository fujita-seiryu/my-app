...
function buildNoteRing() {
  ...
  CHROMATIC_JP.forEach((_, i) => {
    ...
    note.style.left = `${x}%`;
    note.style.top = `${y}%`;
    note.addEventListener("click", () => playReferenceTone(i, note));
    el.noteRing.insertBefore(note, ringCenter);
  });
}

// マイク許可の有無にかかわらず、基準音再生用に共有のAudioContextを用意する
...
// リング上の音名をタップした時に、その音程を実際に鳴らす（お手本の基準音を聞くための機能）
function playReferenceTone(degreeIndex, noteEl) {
  ...
  if (noteEl) {
    ...
    setTimeout(() => noteEl.classList.remove("playing"), 1100);
  }
}
